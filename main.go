...
func main() {
    ...
    s := &http.Server{Addr: addr, Handler: r, ReadHeaderTimeout: 5 * time.Second, IdleTimeout: 60 * time.Second}
    if err := s.ListenAndServe(); err != nil { panic(err) }
}

func headAlias(w http.ResponseWriter, r *http.Request) { serveAlias(w, r, true) }
func getAlias(w http.ResponseWriter, r *http.Request)  { serveAlias(w, r, false) }

func serveAlias(w http.ResponseWriter, r *http.Request, headOnly bool) {
    user := chi.URLParam(r, "user")
    alias := chi.URLParam(r, "alias")
    if !reUser.MatchString(user) || !reAlias.MatchString(alias) { http.Error(w, "bad path", 400); return }
    q := r.URL.Query()
    vraw := q.Has("raw")
    ...
    if dl { w.Header().Set("Content-Disposition", `attachment; filename="`+alias+`"`) }
    if vraw {
        mt := sniff(p)
        w.Header().Set("Content-Type", mt)
        if headOnly { w.WriteHeader(200); return }
        f, err := os.Open(p); if err != nil { http.Error(w, "gone", 410); return }
        defer f.Close()
        http.ServeContent(w, r, "", fileMtime(p), f)
        return
    }
    if headOnly { w.Header().Set("Content-Type", "text/html; charset=utf-8"); w.WriteHeader(200); return }
    src, err := os.ReadFile(p); if err != nil { http.Error(w, "gone", 410); return }
    html, err := runPandoc(src); if err != nil { http.Error(w, "render error", 500); return }
...
func resolvePath(user, alias, ver string) (string, error) {
    ...
    if !strings.HasPrefix(p, filepath.Clean(blobDir)+string(os.PathSeparator)) { return "", os.ErrNotExist }
    return p, nil
}
...
func addCanonical(r *http.Request) []byte {
    u := *r.URL
    q := u.Query()
    if !q.Has("html") && !q.Has("raw") { q.Add("html", "") }
...
func sniff(p string) string {
    f, err := os.Open(p); if err != nil { return "application/octet-stream" }
    defer f.Close()
    buf := make([]byte, 512)
    n, _ := f.Read(buf)
...
func fileMtime(p string) time.Time { st, err := os.Stat(p); if err != nil { return time.Now() }; return st.ModTime() }

func etagFile(p string) string {
    f, err := os.Open(p); if err != nil { return `W/"0"` }
    defer f.Close()
    h := sha256.New()
    _, _ = io.Copy(h, f)
